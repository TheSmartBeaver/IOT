...
typedef struct on/off_switch_instance_struct {
    anjay_iid_t iid;

    // TODO: instance state
} on/off_switch_instance_t;

typedef struct on/off_switch_object_struct {
    const anjay_dm_object_def_t *def;
    AVS_LIST(on/off_switch_instance_t) instances;

    // TODO: object state
} on/off_switch_object_t;

static inline on/off_switch_object_t *
get_obj(const anjay_dm_object_def_t *const *obj_ptr) {
    assert(obj_ptr);
    return AVS_CONTAINER_OF(obj_ptr, on/off_switch_object_t, def);
}

static on/off_switch_instance_t *find_instance(const on/off_switch_object_t *obj,
                                               anjay_iid_t iid) {
    AVS_LIST(on/off_switch_instance_t) it;
...
static int init_instance(on/off_switch_instance_t *inst, anjay_iid_t iid) {
...
static void release_instance(on/off_switch_instance_t *inst) {
    // TODO: instance cleanup
    (void) inst;
}

static on/off_switch_instance_t *
add_instance(on/off_switch_object_t *obj, anjay_iid_t iid) {
    assert(find_instance(obj, iid) == NULL);

    AVS_LIST(on/off_switch_instance_t) created =
            AVS_LIST_NEW_ELEMENT(on/off_switch_instance_t);
    if (!created) {
        return NULL;
    }
    ...
    AVS_LIST(on/off_switch_instance_t) *ptr;
...
static int instance_create(anjay_t *anjay,
                           ...
                           anjay_iid_t iid) {
    (void) anjay;
    on/off_switch_object_t *obj = get_obj(obj_ptr);
    assert(obj);
...
static int instance_reset(anjay_t *anjay,
    ...
    on/off_switch_object_t *obj = get_obj(obj_ptr);
    assert(obj);
    on/off_switch_instance_t *inst = find_instance(obj, iid);
    assert(inst);
...
const anjay_dm_object_def_t **on/off_switch_object_create(void) {
    on/off_switch_object_t *obj = (on/off_switch_object_t *) avs_calloc(1, sizeof(on/off_switch_object_t));
    if (!obj) {
        return NULL;
    }
    obj->def = &OBJ_DEF;
...
void on/off_switch_object_release(const anjay_dm_object_def_t **def) {
    if (def) {
        on/off_switch_object_t *obj = get_obj(def);
